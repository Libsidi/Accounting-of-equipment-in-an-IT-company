...
class PurchasesTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        columns = ('purchase_id', 'vendor_id', 'equipment_id', 'purchase_date', 'cost')
    ...
    def delete_record(self):
        ...
        if not selected_item:
            ...
            return
        purchase_id = self.tree.item(selected_item)['values'][0]
        confirm = messagebox.askyesno("Подтверждение", "Вы уверены, что хотите удалить выбранную закупку?")
        if confirm:
            query = "DELETE FROM purchases WHERE purchase_id = %s"
            try:
                self.cursor.execute(query, (purchase_id,))
                self.conn.commit()
                self.refresh_data()
                messagebox.showinfo("Успех", "Закупка успешно удалена.")
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить запись: {e}")

    def add_record(self):
        add_window = tk.Toplevel(self)
        add_window.title("Добавить закупку")
        ...
        labels = ["ID поставщика:", "ID оборудования:", "Дата покупки (ГГГГ-ММ-ДД):", "Стоимость:"]
        entries = []
        ...
        def save_new_record():
            vendor_id = entries[0].get()
            equipment_id = entries[1].get()
            purchase_date = entries[2].get()
            cost = entries[3].get()

            if not vendor_id or not equipment_id or not cost:
                messagebox.showwarning("Предупреждение", "Пожалуйста, заполните обязательные поля.")
                return

            try:
                query = """
                INSERT INTO purchases (vendor_id, equipment_id, purchase_date, cost)
                VALUES (%s, %s, %s, %s)
                """
                self.cursor.execute(query, (
                    int(vendor_id),
                ...
                self.conn.commit()
                messagebox.showinfo("Успех", "Закупка успешно добавлена.")
                add_window.destroy()
                self.refresh_data()
    ...
    def sort_column(self, col, reverse):
        ...
        if col in ('purchase_id', 'vendor_id', 'equipment_id'):

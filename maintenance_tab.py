...
class MaintenanceTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        columns = ('maintenance_id', 'equipment_id', 'service_date', 'service_details', 'technician')
    ...
    def delete_record(self):
        ...
        if not selected_item:
            ...
            return
        maintenance_id = self.tree.item(selected_item)['values'][0]
        confirm = messagebox.askyesno("Подтверждение", "Вы уверены, что хотите удалить выбранную запись обслуживания?")
        if confirm:
            query = "DELETE FROM maintenance WHERE maintenance_id = %s"
            try:
                self.cursor.execute(query, (maintenance_id,))
                self.conn.commit()
                self.refresh_data()
                messagebox.showinfo("Успех", "Запись обслуживания успешно удалена.")
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить запись: {e}")

    def add_record(self):
        add_window = tk.Toplevel(self)
        add_window.title("Добавить запись обслуживания")
        ...
        labels = ["ID оборудования:", "Дата обслуживания (ГГГГ-ММ-ДД):", "Детали обслуживания:", "Техник:"]
        entries = []
        ...
        def save_new_record():
            equipment_id = entries[0].get()
            service_date = entries[1].get()
            service_details = entries[2].get()
            technician = entries[3].get()

            if not equipment_id:
                messagebox.showwarning("Предупреждение", "Пожалуйста, введите ID оборудования.")
                return

            try:
                query = """
                INSERT INTO maintenance (equipment_id, service_date, service_details, technician)
                VALUES (%s, %s, %s, %s)
                """
                self.cursor.execute(query, (
                    int(equipment_id),
                    service_date if service_date else None,
                    service_details if service_details else None,
                    technician if technician else None
                ))
                self.conn.commit()
                messagebox.showinfo("Успех", "Запись обслуживания успешно добавлена.")
                add_window.destroy()
                self.refresh_data()
    ...
    def sort_column(self, col, reverse):
        ...
        if col == 'maintenance_id' or col == 'equipment_id':

...
class VendorsTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        columns = ('vendor_id', 'name', 'contact_name', 'contact_phone', 'contact_email')
    ...
    def delete_record(self):
        ...
        if not selected_item:
            ...
            return
        vendor_id = self.tree.item(selected_item)['values'][0]
        confirm = messagebox.askyesno("Подтверждение", "Вы уверены, что хотите удалить выбранного поставщика?")
        if confirm:
            ...
            try:
                ...
                self.conn.commit()
                self.refresh_data()
                messagebox.showinfo("Успех", "Поставщик успешно удалён.")
            except Exception as e:
    ...
    def add_record(self):
        ...
        labels = ["Название:", "Контактное лицо:", "Телефон:", "Email:"]
        entries = []
        ...
        def save_new_record():
            name, contact_name, contact_phone, contact_email = [entry.get().strip() for entry in entries]
            ...
            try:
                ...
                VALUES (%s, %s, %s, %s)
                """
                self.cursor.execute(query, (name, contact_name or None, contact_phone or None, contact_email or None))
                self.conn.commit()
